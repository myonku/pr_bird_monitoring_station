...
CREATE TABLE IF NOT EXISTS business_edge_events (
  ...
  station_entity_id CHAR(36) NOT NULL,
  occurred_at DATETIME(3) NOT NULL,
  received_at DATETIME(3) NOT NULL,
  payload_version VARCHAR(32) NOT NULL,
  payload_type VARCHAR(32) NOT NULL,
  payload_mongo_ref VARCHAR(128) NOT NULL,
  transport_meta JSON NOT NULL,
  metadata JSON NOT NULL,
  created_at DATETIME(3) NOT NULL,
  updated_at DATETIME(3) NOT NULL,
  PRIMARY KEY (event_id),
  KEY idx_business_edge_events_station_time (station_entity_id, occurred_at),
  KEY idx_business_edge_events_received (received_at),
  CONSTRAINT fk_business_edge_events_station FOREIGN KEY (station_entity_id) REFERENCES business_entities(entity_id)
...
CREATE TABLE IF NOT EXISTS business_processing_jobs (
  ...
  KEY idx_business_jobs_station_status (station_entity_id, status),
  ...
  CONSTRAINT fk_business_jobs_station FOREIGN KEY (station_entity_id) REFERENCES business_entities(entity_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS business_recognition_results (
  ...
  CONSTRAINT fk_business_recognition_species FOREIGN KEY (species_entity_id) REFERENCES business_entities(entity_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS business_monitoring_records (
  ...
  KEY idx_business_records_station_time (station_entity_id, captured_at),
  ...
  CONSTRAINT fk_business_records_station FOREIGN KEY (station_entity_id) REFERENCES business_entities(entity_id),
  CONSTRAINT fk_business_records_species FOREIGN KEY (species_entity_id) REFERENCES business_entities(entity_id),
