...
- 非认证中心模块（gateway、api_service）不本地管理 challenge/session/token/grant 状态，统一转发认证中心门面。
- 限流决策基于认证中心返回的身份上下文执行。
...
6. 运行期认证校验、续期、撤销统一转发认证中心，异常时回退到重新 bootstrap。
...
3. 将自身注册至服务发现中心。
4. 装配 gRPC handler 与拦截器链。
5. 启动 gRPC server 对内提供业务能力。
6. 进入稳定运行，处理业务请求与跨服务调用。
