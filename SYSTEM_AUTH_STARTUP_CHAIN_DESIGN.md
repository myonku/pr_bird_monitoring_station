...
3. 将自身注册至服务发现中心。
4. 预热关键下游安全通道；若未预热，则在首跳转发前执行 EnsureChannel 握手。
5. 启动 HTTP 入站服务。
6. 进入稳定运行，持续执行加密转发、限流、鉴权、下游认证上下文透传与失败恢复。
...
6. 内部转发默认执行回源认证中心校验（至少会话校验，按场景补充令牌校验）；跨服务出站调用前需确保安全通道可用（预热复用或首跳握手）。
...
6. 进入稳定运行，处理业务请求与跨服务调用，并在已建立安全通道上发起调用，基于回源复核后的身份执行限流。
...
6. 入站任务处理默认执行回源认证中心校验（至少会话校验，按场景补充令牌校验）；跨服务出站调用前需确保安全通道可用（预热复用或首跳握手）。
...
6. 进入稳定运行，处理任务与跨服务调用，并在已建立安全通道上发起调用，基于回源复核后的身份执行限流。

说明：

- Data Worker 在认证主干上与 API Service 保持一致；差异仅在业务入口形态（任务消费/异步处理）。
