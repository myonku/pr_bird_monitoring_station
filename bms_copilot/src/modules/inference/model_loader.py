...
from src.iface.inference.inference import IModelBundleLoader
from src.models.inference.common import ModelTier, TaskType
from src.models.inference.config import InferenceConfig
from src.models.inference.workflow import (
    InferenceModelArtifact,
...
class LocalModelBundleLoader(IModelBundleLoader):
    ...
    @staticmethod
    def _iter_label_file_candidates(
        label_dir: Path, configured_name: str
    ) -> list[Path]:
    ...
    def _resolve_labels_for_task(
        self, config: InferenceConfig, task: str
    ) -> list[str]:
        if task == "detection":
            label_file_name = config.detection_label_file_name.strip()
        else:
            label_file_name = config.classification_label_file_name.strip()
    ...
    def _find_candidate_spec(
        ...
        *,
        task: str,
        model_file_name: str,
        candidates: list[InferenceModelCandidateSpec],
    ) -> InferenceModelCandidateSpec:
        matched = [
            item
            for item in candidates
            if item.task == TaskType(task) and item.file_name == model_file_name
        ]
        if not matched:
            expected = sorted(
                item.file_name for item in candidates if item.task == TaskType(task)
            )
            raise ValueError(
                f"no candidate config matched for task={task}, file={model_file_name}; expected one of: {expected}"
            )
        if len(matched) > 1:
            raise ValueError(
    ...
    def _load_detection_handle(
        ...
        *,
        path: Path,
        spec: InferenceModelCandidateSpec,
        labels: list[str],
    ) -> dict[str, Any]:
    ...
    def load(self, config: InferenceConfig) -> LoadedInferenceBundle:
        if not config.standard_candidates:
        ...
        classification_path = self._discover_single_model_file(
            Path(config.classification_dir),
            "classification",
        )
        ...
        detection_spec = self._find_candidate_spec(
            ...
            candidates=config.standard_candidates,
        )
        ...
        detection_labels = self._resolve_labels_for_task(config, "detection")
        classification_labels = self._resolve_labels_for_task(config, "classification")
        ...
        self._bundle = LoadedInferenceBundle(
            contract=contract,
