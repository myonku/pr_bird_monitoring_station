...
class MockMonitoringClient implements MonitoringClient {
	...
	Future<ClientAuthCredentialsResponse> refreshSession(
		...
		return ClientAuthCredentialsResponse(
			...
			downstreamToken: 'mock-downstream-refreshed-${now.millisecondsSinceEpoch}',
			tokenType: 'access',
	...
	Future<ClientDashboardSnapshotResponse> fetchDashboardSnapshot(
		...
		return ClientDashboardSnapshotResponse(
			...
			recentRecords: snapshot.recentRecords.map(_toBirdRecordResponse).toList(
				growable: false,
			),
		);
	}
	...
	Future<ClientWeeklyTrendResponse> getWeeklyTrend(
		...
		final total = points.fold<int>(0, (current, point) => current + point.value);
	...
	Future<ClientRangeSummaryResponse> getRangeSummary(
		...
		final peakPoint = distribution.isEmpty
				? null
				: distribution.reduce((left, right) => left.value >= right.value ? left : right);
	...
	List<ClientSpeciesShareResponse> _buildSpeciesShares(List<BirdRecord> records) {
		if (records.isEmpty) {
			return const [];
		}
