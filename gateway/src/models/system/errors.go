...
var (
	...
	ErrAuthAuthorityChannelDependenciesRequired Error = *NewError(AuthErr, "auth authority channel dependencies are required")
	ErrDownstreamGrantClientRequired            Error = *NewError(AuthErr, "downstream grant client is required")
	ErrModuleCredentialDependenciesRequired     Error = *NewError(AuthErr, "module credential manager dependencies are required")
	ErrModuleCredentialBootstrapRequestNeeded   Error = *NewError(AuthErr, "module credential bootstrap request is required")
...
var (
	...
	ErrChannelRequiredForEncryptedPayload Error = *NewError(CommSecErr, "encrypted payload requires an active secure channel")
