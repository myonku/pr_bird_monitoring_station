...
func (c *ExternalAuthRPCClient) ForwardRefreshTokenBundle(
	ctx context.Context,
	...
	client := authv1.NewAuthAuthorityExternalAuthServiceClient(conn)
	callCtx, callCancel := context.WithTimeout(ctx, c.callTimeout)
	resp, err := client.ForwardRefreshTokenBundle(
		callCtx,
	...
	callCancel()
	if err != nil {
		return nil, fmt.Errorf("forward token refresh rpc failed: %w", err)
	}
