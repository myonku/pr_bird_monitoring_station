...
- development：当前默认开发联调链路（含认证中心、回源认证与限流约束）。
- no-auth：后端最小业务测试模式，默认屏蔽认证中心、短路认证链、禁用限流；认证中心若被拉起，应在判定 no-auth 后自主停止，作为当前暂时性设计方向。
...
| 限流器                       | 启用                 | 关闭                 | 不适用       | 不适用                     |
| 认证中心进程                 | 启动并对外提供服务   | 默认不启动；若被拉起则在判定 no-auth 后自主停止 | 不适用       | 不适用                     |
...
3. 跳过额外下游准备。
4. 装配 no-auth 转发准备器（保持统一转发结构，但不执行鉴权）。
5. 关闭限流链后启动入站服务。
...
1. 初始化 auth/ratelimit/orchestration 组件。
2. 启动认证与通道能力接口。
3. 进入认证与通信安全请求处理循环。
...
1. 认证中心默认不启动。
2. 若进程被拉起，首先读取配置并判定 run_mode=no-auth。
3. 一旦确认 no-auth，立即自主停止对外服务或直接退出，不进入认证签发主链。
4. 关闭认证签发、校验、刷新、撤销主链路。
5. 关闭限流链路。
6. 关闭额外下游准备与应用层加密链路。
7. 如需保留占位进程，仅允许健康检查壳，不暴露认证接口。

### 6.3 行为开关

启用：

- 最小可用进程能力（仅在被拉起时保留健康检查壳）。

禁用：

- challenge/bootstrap/token/session。
- 限流决策。
- 额外下游准备、通道管理、加解密编排。
...
- 认证中心的 no-auth 处理仅是暂时性设计方向：默认不启动；若被拉起，必须在判定 run_mode=no-auth 后自主停止或仅保留健康检查壳，不进入认证链路。
- 进入 development 或更高环境前，必须恢复认证链路与限流链路。
...
4. 初始化本地密钥服务并解析有效 bootstrap 引用ID（优先 active_key_id，缺失时回退 instance_id；供实例元数据与后续模式切换复用）。
5. no-auth 分支处理：
	- gateway/data_worker：跳过 bootstrap 调用。
	- certification_server：默认不启动；若因部署策略启动，则在判定 no-auth 后自主停止或仅保留健康检查壳，不进入认证链路。
