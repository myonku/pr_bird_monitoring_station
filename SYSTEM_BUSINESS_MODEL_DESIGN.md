...
- 面向客户端的业务接口模块后续单独引入，不与 `data_worker` 混为一个职责。
...
1. 边缘端采集画面、触发抓拍，并组装一次上传事件。
2. 边缘端根据负载、网络、置信度或策略决定：本地识别后上传，还是只上传原始事件交给后端处理。
3. 事件经由网关进入系统，网关负责路由、认证和目标服务选择。
4. `data_worker` 接收事件后先做解析和校验，再决定是否执行通用模型推理或补充识别。
5. `data_worker` 将归一化后的业务结果写入数据库，并在必要时生成聚合数据。
6. 未来的客户端业务模块直接读取这些业务结果和聚合结果，对外提供查询接口。
7. Flutter 客户端登录后读取首页概览、记录列表、记录详情和统计数据，并展示给普通用户。
...
4. `DashboardSnapshot` / `RecordDetailView` / `TrendPoint` / `SpeciesShare`：面向客户端的读模型。

其中，`MonitoringRecord` 是未来最该稳定下来的核心业务实体；其余模型都应该围绕它派生。
